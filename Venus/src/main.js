...
import { applyCloudAppearance } from './clouds.js';

const SETTINGS = {
  rotationSpeed: -0.012, // Retrograde; one turn in about 8 minutes 44 seconds at 1×.
  exposure: 1.08,
  sunlight: 3.2,
  fill: 0.025,
...
const canvas = document.querySelector('#venus');
...
async function init() {
  ...
  // Inspected NASA's cloud-covered model, distinct from its radar surface model.
  // Retain NASA's sphere; the reference-inspired cloud artwork is credited separately.
  const gltf = await new GLTFLoader().loadAsync(`${import.meta.env.BASE_URL}models/venus.glb`);
  ...
  model.traverse(object => {
    ...
    for (const material of materials) {
      // An opaque, matte cloud deck; no rocky relief, metallic sheen, or emission.
      material.roughness = 1;
      material.metalness = 0;
      if (material.map) material.map.anisotropy = renderer.capabilities.getMaxAnisotropy();
    }
  });
  await applyCloudAppearance(model, renderer);
  ...
  canvas.addEventListener('webglcontextlost', event => {
    ...
    showError('The graphics connection was interrupted. Reload to restore Venus.');
  });
  reset();
  resize();
...
init().catch(error => {
  console.error('Venus could not start:', error);
  showError('Venus could not load. Reload the page; if this persists, check that WebGL is enabled and the local model and cloud texture are available.');
});
